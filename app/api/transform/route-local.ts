...
import { togetherVercelAiClient, createUnifiedLLMClient, checkOllamaHealth } from "@/lib/apiClients";
...
export async function POST(req: NextRequest) {
  ...
  // Check for local LLM preference in headers or environment
  const useLocalLLM = req.headers.get("X-Use-Local-LLM") === "true" || 
  ...
  // Check if we should use local LLM and if it's available
  let aiClient;
  let model;
  ...
  if (useLocalLLM) {
    try {
      ...
      if (healthCheck.status === 'healthy') {
        ...
        // The local client will auto-select model based on transcription language
    ...
    console.log("Using Together.ai service");
    aiClient = togetherVercelAiClient(apiKey);
    model = aiClient("meta-llama/Meta-Llama-3-70B-Instruct-Turbo");
  }
    ...
    async start(controller) {
      try {
        ...
        await prisma.transformation.update({
          where: { id: transformation.id },
          data: { 
            text: "Error occurred during text generation", 
            isGenerating: false 
          },
        });
  ...
  return new Response(stream, {
    headers: {
      ...
      "X-LLM-Service": useLocalLLM ? "local-ollama" : "together-ai",
    },
  });
}